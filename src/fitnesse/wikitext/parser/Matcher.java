...
public class Matcher {
    ...
    public Matcher whitespace() {
        ...
        matches.add((input, symbols, offset) -> {
    ...
    public Matcher startLine() {
        matches.add((input, symbols, offset) -> isStartLine(input, symbols, offset) ? new Maybe<>(0) : Maybe.noInteger);
        return this;
    }

    public Matcher startLineOrCell() {
        matches.add((input, symbols, offset) -> isStartLine(input, symbols, offset) || isStartCell(symbols)
    ...
    public Matcher string(final String delimiter) {
        ...
        matches.add((input, symbols, offset) -> input.matches(delimiter, offset) ? new Maybe<>(delimiter.length()) : Maybe.noInteger);
        return this;
    }

    public Matcher listDigit() {
        firstIsDigit('1');
        matches.add((input, symbols, offset) -> isDigitInput('1', input, offset) ? new Maybe<>(1) : Maybe.noInteger);
        return this;
    }
    ...
    public Matcher ignoreWhitespace() {
        matches.add((input, symbols, offset) -> new Maybe<>(input.whitespaceLength(offset)));
        return this;
    }
